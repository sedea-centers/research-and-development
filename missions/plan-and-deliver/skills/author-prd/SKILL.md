...
On spawned lanes, put **`mission_control_present_structured_choice`** on **line 1** and **`mission_control_send_agent_result`** on the **last line** of the same message when the turn ends (rule **2** § *Same message as spawn terminal*). Stop after the MCP result call. Do not spawn downstream planning agents from this skill.
