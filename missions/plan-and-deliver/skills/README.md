...
**Checkpoint trust (`trustLevel: checkpoint`):** Auto-advance scripted happy-path steps without a turn-end modal; emit structured choice only at **USER_CHECKPOINT** markers, **implicit external-wait** surfaces, or **exception** paths. **Developer-input** gates remain USER_CHECKPOINT when Checkpoint auto-advance criteria fail — **not** optional and **not** external-wait when the developer must pick to continue. Happy-path auto-advance covers review feedback, create-PR follow-ups, post-create-pr → delegate-merge review, rebase push, pre-merge confirm, and **`pr-review`** disposition (CI/Must) when clean.
...
**Ship and ops skills:** **`coding-session`** (Checkpoint § *Checkpoint turn UX* — auto-advance happy path including review feedback, create-PR follow-ups, post-create-pr → delegate-merge review, rebase `--force-with-lease`, pre-merge **`delegate-merge-confirm`**; **`USER_CHECKPOINT`** at worktree-open when layer 2 required, implementation continuation (exception), repo rules reconciliation (exception), ship cut-point (exception), and After deploy manual steps; center **`worktree-setup.sh`** bootstrap hints; **pre-PR ship gate** — no push/create-PR modals until **`pre-pr-review`** **`go`** except executive override; **auto** pre-PR spawn after cut-point + Before deploy; **auto** inline **create-pr** on clean **go**; inline **`pr-review`** (Checkpoint auto-disposition for CI/Must); **agent-delegated approve + merge** when authorized; **auto** post-merge cleanup when merged; inline **deploy-walk**; inline **plan-reconcile**), **`worktree-bootstrap`** (**deprecated** — exception-only inline retry; normative bootstrap is center setup on **`coding-session`**), **`pre-pr-review`** — reviewer lane only; no ship-chain gates (findings hand back to **`coding-session`** [Review feedback approval gate](../coding-session/SKILL.md#review-feedback-approval-gate)). **`pr-review`** Step **4** — disposition gate uses **contextual** `options` from triage counts (see § *Build disposition options*). Prefer **AskQuestion** or **`MC_PHASED_RESPONSE_V1`** when recap and modal belong in one message. Gate detail: **`coding-session/SKILL.md`** § *Checkpoint turn UX*, § *Repo rules reconciliation gate*, § *Pre-PR ship gate (push/PR)*, § *Post-create-pr handoff gate*, and § *Implementation continuation gate*.
