...
**Center governance CI (software-development center repo).** GitHub Actions workflow **`.github/workflows/center-governance.yml`** runs on every PR and push to **`main`** on **`sedea-centers/software-development`**. The job checks out the center repo, installs script dependencies under **`missions/plan-and-deliver/scripts/`**, then runs **`verify-skill-manifest.mjs`** (including **`--enforce-spawn-byte-budget`** for **planning skills only** — ship skills stay WARN-only until Phase B), **`verify-checkpoint-steps.mjs`**, and **`verify-no-legacy-wire-tokens.mjs`**. Phase A excludes **`verify-lane-warmup-parity.mjs`** (Phase B fixture).
...
**Spawn warm-up byte budget.** `verify-skill-manifest.mjs` sums on-disk bytes for each spawned skill's frontmatter **`warmUpRules` ∪ `laneRules`**, emits **`WARN:`** lines when over **384 KiB**, and reports **`spawn byte budget smoke`** in the OK line. Pass **`--enforce-spawn-byte-budget`** to exit **1** on over-cap **planning** spawn skills only (Phase A — **`coding-session`** and other ship skills stay WARN-only until Phase B). See **`skills/README.md`** § *Warm-up cap exceptions*.
...
flowchart TD
 ...
 APR --> SEED[§4 seed → §5 spawn master-planner]
...
| **§3 Author PRD** | Child lane | Full or updated PRD under **`.sedea/operations/.../docs/`** (via **`operationsDocsDirectory`**, or managed existing **`targetPath`**) via **`author-prd`** | Developer approves on **Author PRD** lane → Squad Leader **auto-chains** §4 seed + §5 **`master-planner`** same turn |
...
Operational documents (PRDs, brainstorm reports, ad-hoc PRDs, triage reports) are written under **workspace scope-level `docs/`**:
...
Sedea **`.plan.md`** files are **delivery anchors** for Mission Control ship (Master Plan → PR plans → **`coding-session`**). **Scope and backlog** live in **external PM tools** (JIRA, Trello, ClickUp, Asana, …); traceability uses optional **`Related:`** entries in the **`master-planner`** seed — **not** a Sedea parent tree or Hub roadmap topic.
...
4. **Architectural design.** One or more diagrams showing what the implementation will look like. Pick the diagram type(s) that best fit the feature:
 ...
 When drafting Mermaid, follow [`.sedea/centers/sedea/docs/mermaid-authoring.md`](.sedea/centers/sedea/docs/mermaid-authoring.md) — opaque ids (no reserved bare ids; never use uppercased reserved abbreviations such as `OPT`/`ALT`/`END`/`LOOP` as bare participant/node ids — prefer opaque id + label), sequence `Note` on a single physical line without HTML `<br/>`, and flowchart-only `<br/>` in quoted node labels.
...
| **`master-planner`** Master Plan child | **Yes** — inline **`pr-plan`** §5d on **this lane** after §5c | Owns planning + §5d spawn |
...
On a **`plan and deliver`** Mission Control dispatch, the Squad Leader **§8** ship ledger does **not** show detached child **`mission_control_send_agent_result`** on the leader chat. Progress reaches §8 **only** through Mission Control **host sync** (see **`plan.mdc`** §8 *Mission Control host sync*):
